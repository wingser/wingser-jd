	...
	if ($.JdFarmProdName != "") {
		if ($.JdtreeEnergy != 0) {
			if ($.treeState === 2 || $.treeState === 3) {
				ReturnMessage += `【东东农场】${$.JdFarmProdName} 可以兑换了!,${$.farmTotalEnergy}水滴)\n`;
				TempBaipiao += `【东东农场】${$.JdFarmProdName} 可以兑换了!,${$.farmTotalEnergy}水滴)\n`;
				if (userIndex2 != -1) {
				...
				if ($.JdwaterD != 'Infinity' && $.JdwaterD != '-Infinity') {
					ReturnMessage += `【东东农场】${$.JdFarmProdName}(${(($.JdtreeEnergy / $.JdtreeTotalEnergy) * 100).toFixed(0)}%,${$.JdwaterD}天,${$.farmTotalEnergy}水滴)\n`;
				} else {
					ReturnMessage += `【东东农场】${$.JdFarmProdName}(${(($.JdtreeEnergy / $.JdtreeTotalEnergy) * 100).toFixed(0)}%),${$.farmTotalEnergy}水滴)\n`;
			...
			if ($.treeState === 0) {
				...
				if (userIndex2 != -1) {
					WarnMessageGp2 += `【账号${IndexGp2} ${$.nickName || $.UserName}】水果领取后未重新种植! (东东农场),${$.farmTotalEnergy}水滴)\n`;
				}
				if (userIndex3 != -1) {
					WarnMessageGp3 += `【账号${IndexGp3} ${$.nickName || $.UserName}】水果领取后未重新种植! (东东农场),${$.farmTotalEnergy}水滴)\n`;
				}
				if (userIndex4 != -1) {
					WarnMessageGp4 += `【账号${IndexGp4} ${$.nickName || $.UserName}】水果领取后未重新种植! (东东农场),${$.farmTotalEnergy}水滴)\n`;
				}
				if (userIndex2 == -1 && userIndex3 == -1 && userIndex4 == -1) {
					allWarnMessage += `【账号${IndexAll} ${$.nickName || $.UserName}】水果领取后未重新种植! (东东农场),${$.farmTotalEnergy}水滴)\n`;
				}
					...
					if (safeGet(data)) {
						$.farmInfo = JSON.parse(data)
							if ($.farmInfo.farmUserPro) {
								...
								$.JdwaterD = waterD;
								$.farmTotalEnergy = $.farmInfo.farmUserPro.totalEnergy; // 个人剩余水滴。
							}
					}
				}
			} catch (e) {
